...
const IdeaDetails: React.FC = () => {
  ...
  // Defensive: Only calculate if idea is loaded
  ...
  // Show loading spinner/message if loading or idea is not yet loaded
  if (loading || !idea) {
    return (
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden">
              ...
              <div className="p-6">
                ...
                <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-3">{idea.title || 'Untitled Idea'}</h1>
                ...
                <div className="flex items-center space-x-4 mt-4">
                  ...
                  <button
                    className={`btn btn-outline flex items-center ${idea && savedIdeas.includes(idea.id) ? 'text-primary-600' : ''}`}
                    onClick={handleToggleSave}
                    disabled={saving}
                  >
                    <Bookmark className="h-4 w-4 mr-2" fill={idea && savedIdeas.includes(idea.id) ? '#2563eb' : 'none'} />
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.isArray(idea.tags) && idea.tags.length > 0 ? idea.tags.map((tag: string) => (
                  <span
                    key={tag}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="mb-4">
                <div className="flex items-baseline justify-between">
                  <div>
                    <span className="text-2xl font-bold text-gray-900">${idea.currentFunding?.toLocaleString?.() ?? '0'}</span>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="flex items-center mb-4">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium text-gray-900">{idea.creatorName || 'Unknown'}</p>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-medium">{createdDate || 'Unknown'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Category</span>
                  <span className="text-sm font-medium">{idea.category || 'N/A'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">Stage</span>
                  <span className="text-sm font-medium capitalize">{idea.stage || 'N/A'}</span>
                </div>
                <div className="flex justify-between items-center">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-4">
                {Array.isArray(mockIdeas) && idea.category ? (
                  mockIdeas
                    ...
                    .map((similarIdea) => (
                      ...
                      >
                        <div className="flex items-center">
                          <img
                            src={similarIdea.imageUrl || 'https://via.placeholder.com/64?text=No+Image'}
                            alt={similarIdea.title || 'Idea'}
                            className="w-16 h-16 rounded object-cover"
                          />
                          <div className="ml-3">
                            <h4 className="text-sm font-medium text-gray-900 group-hover:text-primary-600">
                              {similarIdea.title || 'Untitled'}
                            </h4>
                            <p className="text-xs text-gray-500">
                              ${similarIdea.currentFunding?.toLocaleString?.() ?? '0'} raised
                            </p>
                          </div>
                        </div>
                      </Link>
                    ))
                ) : <span className="text-gray-500 text-sm">No similar ideas found.</span>}
              </div>
            </div>
          </div>
        </div>
